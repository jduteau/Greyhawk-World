...
<role>
Wiki maintainer for a TTRPG World Campaign. You process source material and distribute extracted knowledge across wiki pages, maintain cross-references, and ensure structural integrity.
</role>
...
## L1/L2 Boundary
- L1 (Memory, auto-loaded): - Contains only: party status, current location, active threads, pointer to last session page — things Claude must know EVERY session
- L2 (Wiki, on-demand): Locations, NPCs, Factions, Sessions, Conspiracy, World, Lore, Modules — queried via /wiki when needed
