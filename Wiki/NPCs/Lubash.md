---
type: entity
entity-type: npc
name: "Lubash"
status: active
source: T1
confidence: canon
npc_status: alive
npc_scope: local
last_location: "[[Wiki/Locations/Moat-House]]"
allegiance_known: "Unknown — not yet encountered"
allegiance_true: "Chaotic — mercenary ogre; serves Lareth for food and treasure"
faction: "[[Wiki/Factions/Temple-of-Elemental-Evil]]"
first_session: 0
gm_only: true
created: 2026-04-25
updated: 2026-04-25
---

# Lubash

Named ogre guarding room 7 of the [[Wiki/Locations/Moat-House]] dungeon level. Recruited into Temple service by [[Wiki/NPCs/Lareth-the-Beautiful]] with the promise of food (including human flesh) and treasure.

**Stats:** AC 5 · HD 5+1 · HP 21 · Move 9" · Bardiche (2-8 +5 = 7–13 damage).

## Description

A large, hungry ogre with a nest of old clothing and skins in the corner of his room. His treasure hoard is mostly worthless: glass beads, brass candlesticks, and trash — plus 823 cp, 46 sp, 3 gp. By some twist of fate, a small-sized elven cloak is intermixed in the carpet and rags of his nest.

## Role

Guardian of the central dungeon passage. He has been instructed to allow only those wearing the **yellow eye of fire** symbol OR who know the proper **secret sign** (crossed arms before the face). He will not attack immediately — he waits to see if the challenge is answered. If it is not, he attacks.

He will devour anyone he kills at the first opportunity. He tosses wounded and prisoners into the adjoining room (area 8, his pantry) for later consumption. He will pursue fleeing parties, but no further than the stairs up (area 4, above).

## Personality & Voice

- Does not speak Common fluently — communicates primarily through posture and gesture when issuing challenges; the crossed-arms challenge sign is expected and he waits for it
- Hungry and territorial; his entire disposition communicates that this passage is his and anything that enters is either welcome (correct sign) or food (incorrect sign)
- Will pause before attacking — there is a brief, assessing moment where he waits for the sign; this can read as hesitation but is actually ritual
- No ideology, no loyalty beyond the current arrangement; he is here because the food is regular; once the arrangement ends (Lareth is gone), he will leave or become entirely feral

## Goals

- Eat regularly; the arrangement with Lareth provides human flesh and he wants it to continue
- Guard the central passage as instructed; he takes the challenge ritual seriously because it defines his role
- Accumulate treasure in his nest; he covets shiny things without understanding their value
- Survive; he will pursue fleeing parties but not past the stairs, which are the edge of his territory

## Knowledge

- Knows the correct challenge sign (crossed arms before the face) and the yellow eye of fire symbol
- Knows "the Master" is somewhere to the south; no further detail
- Knows his pantry (area 8) currently holds prisoners; will refer to them as food
- Knows the dungeon's central passage layout but nothing about what lies beyond his territorial range

## Memories

*Has not yet met the party. No shared history.*

## Character Relationship

*No character-specific relationships established yet.*

## GM Notes

*gm_only*

- Area 8 (his pantry) currently contains two merchants (mildly tortured, will promise 100 sp each as reward, paid via caravan to Hommlet four weeks after release) and a gnome caught spying by gnolls (will give a plain iron ring — recognized 70% of the time by gnomes within 100 miles as a sign of alliance).
- The elven cloak in his nest requires active searching; it is not immediately visible.
- Lubash is aware that "the Master" is somewhere to the south, but does not know details.

## History

No session history yet.

## Cross-References

- [[Wiki/Locations/Moat-House]]
- [[Wiki/NPCs/Lareth-the-Beautiful]]
- [[Wiki/Factions/Temple-of-Elemental-Evil]]
- [[Wiki/Modules/T1-Village-of-Hommlet]]
